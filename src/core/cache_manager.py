...
class CodeCacheManager:
    ...
    async def lookup(
        ...
        """
        Lookup code in cache (calculates cache key internally).
        ...
        # Delegate to lookup_by_key
        return await self.lookup_by_key(cache_key)
    ...
    async def lookup_by_key(
        ...
        This method is used when the cache key has been calculated at the
        beginning of node execution to ensure consistency between lookup and save.
    ...
    async def save(
        ...
        """
        Save generated code to cache (calculates cache key internally).
        ...
        # Delegate to save_with_key
        return await self.save_with_key(
            cache_key=cache_key,
            task_hash=task_hash,
    ...
    async def save_with_key(
        ...
        context_schema_hash: Optional[str] = None,
        workflow_id: Optional[int] = None,
        node_id: Optional[str] = None
    ) -> CodeCache:
        """
        Save generated code to cache using pre-calculated cache key.
        ...
        Args:
            ...
            tokens_used: Total tokens consumed
            cost_usd: Cost in USD
            task_hash: Optional pre-calculated task hash
            context_schema_hash: Optional pre-calculated context schema hash
            workflow_id: Optional workflow ID
            node_id: Optional node ID
        ...
        # Generate task_hash if not provided
        if task_hash is None:
            task_hash = generate_task_hash(original_prompt)

        # Context schema hash is optional (can be null)
        # If not provided, leave as None
    ...
    async def record_success(
        ...
        logger.debug(
            f"✅ Cache success recorded: {cache_key[:16]}... "
    ...
    async def record_failure(
        ...
        # Log warning
        logger.warning(
            f"❌ Cache failure recorded: {cache_key[:16]}... "
        ...
        if entry.success_rate < 0.7 and entry.times_reused >= 3:
            logger.error(
                f"🗑️  Cache entry unreliable (success rate: {entry.success_rate:.1%}). "
    ...
    async def delete(self, cache_key: str):
        ...
        if entry:
            ...
            self.db.commit()
            logger.info(f"🗑️  Deleted cache entry: {cache_key[:16]}...")
        else:
            logger.warning(f"Cache entry not found: {cache_key[:16]}...")
    ...
    async def cleanup_old_entries(self, days_old: int = 90):
        ...
        logger.info(f"🧹 Cleaned up {deleted} old cache entries (>{days_old} days old)")
